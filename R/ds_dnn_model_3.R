...
ds_dnn_model_3 <- function(out,hnodes, epochs=10,
                         batch_size=32, activation="relu", add_dropout=TRUE,
                         pct_dropout=0.2,name_mod="mod", batchnorm=TRUE, lr=0.001,
  ...
  if (batchnorm) {
    model <- layer_batch_normalization(model,name = paste(name_mod,"batch_normalization_1",sep="_"))
  }
  ...
  for (i in 1:N) {
    model <- model %>%
      layer_dense(units=hnodes[i], activation=activation, input_shape = c(n_features),
                  kernel_constraint=ds$w_norm, name = paste(name_mod,"dense",i,sep="_"),
                  kernel_regularizer=regularizer_l1_l2(l1=l1, l2=l2))
    if (add_dropout) {
      model <- layer_dropout(model, rate=pct_dropout,name = paste(name_mod,"dropout",i,sep="_"))
    }
    if (batchnorm) {
      model <- layer_batch_normalization(model,name = paste(name_mod,"batch_normalization",i+1,sep="_"))
    }
