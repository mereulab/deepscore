...
#' @param metric The evaluation metric used to calculate the feature importance, "loss" or "accuracy".
...
ds_feature_importance<-function(X,Y,model,metric,features,max_n=1000){
  ...
  if(metric=="accuracy"){
    ...
    for (i in 1:num_permutations) {
      xi = rep(0,nrow(X))
      Xi = cbind(xi, X[,-which(colnames(X) == features[i])])
      permuted_metrics <- mod1 %>% evaluate(x = Xi, y = Y,verbose = 0)
